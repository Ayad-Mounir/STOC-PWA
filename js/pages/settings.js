...
function DriveTab(){
  ...
  function withToken(callback){
    ...
    if(!cid){setMsg("⚠️ لم يتم إعداد Google Drive — تواصل مع المدير");setStatus("err");return;}
    ...
    try{
      var client=google.accounts.oauth2.initTokenClient({
        ...
        callback:function(resp){
          if(resp.error){setStatus("err");setMsg("❌ فشل تسجيل الدخول: "+resp.error);return;}
  ...
  // ── زر الرفع ──
  function handleSave(){
    setStatus("busy");setMsg((window.__i18n?window.__i18n.t("جارٍ الرفع..."):"جارٍ الرفع..."));
    withToken(async function(tk){
      try{
        ...
        setLastSync(now);setStatus("ok");setMsg("✅ تم الحفظ على Drive بنجاح");
        saveDriveCfg({...loadDriveCfg(),lastSync:now,fileId:fid});
      }catch(e){setStatus("err");setMsg("❌ خطأ في الرفع: "+e.message);}
    });
  }

  // ── زر الاسترجاع ──
  function handleLoad(){
    if(!confirm("⚠️ سيتم استبدال البيانات الحالية بالنسخة الاحتياطية. تابع؟"))return;
    setStatus("busy");setMsg((window.__i18n?window.__i18n.t("جارٍ الاسترجاع..."):"جارٍ الاسترجاع..."));
    withToken(async function(tk){
      try{
        var fid=fileId;
        if(!fid){
          var s=await fetch("https://www.googleapis.com/drive/v3/files?q=name='"+DRIVE_FILE_NAME+"'+and+trashed=false&fields=files(id)",{headers:{Authorization:"Bearer "+tk}});
          var r=await s.json();
          if(!r.files||r.files.length===0){setMsg("⚠️ لا توجد نسخة احتياطية بعد");setStatus("idle");return;}
  ...
  return React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:12}},
    ...
    lastSync&&React.createElement("div",{style:{color:"#6b7280",fontSize:10,textAlign:"center"}},"🕐 آخر نسخة: ",lastSync),
    ...
    !hasClientId&&React.createElement("div",{style:{padding:"10px 14px",background:"#1c1007",border:"1px solid #92400e",borderRadius:10,fontSize:11,color:"#fbbf24",textAlign:"center",lineHeight:1.7}},
      "⚠️ لم يتم إعداد Google Drive بعد",React.createElement("br",null),"تواصل مع المدير لتفعيل هذه الخاصية")
  );
}
...
function AISettingsTab(){
  ...
  function saveAll(){
    ...
    if(arr.length===0){setMsg("⚠️ أدخل مفتاحاً واحداً على الأقل");return;}
    saveKeys(arr);
    setKeys(arr);
  ...
  function clearAll(){
    if(!confirm("حذف كل مفاتيح Gemini؟"))return;
  ...
  return React.createElement("div",null,
    ...
    React.createElement("div",{style:{display:"flex",alignItems:"center",gap:10,padding:10,
      ...
      React.createElement("div",null,
        React.createElement("div",{style:{color:keys.length>0?"#3fb950":"#f87171",fontWeight:700,fontSize:12}},
          keys.length>0?keys.length+" مفتاح مفعّل (المستخدم الآن: #"+(currentIdx+1)+")":"لا يوجد مفتاح — الذكاء الاصطناعي معطل"),
        React.createElement("div",{style:{color:"#6b7280",fontSize:10,marginTop:1}},
          keys.length>0?"عند انتهاء أحدهم يتحول تلقائياً للتالي":"أضف مفتاحاً للاستفادة من التحليل الآلي"))),
      ...
      return React.createElement("div",{key:i,style:{background:"#161b22",border:"1px solid "+(vals[i].trim()?"#30363d88":"#30363d"),borderRadius:10,padding:12,marginBottom:10}},
        React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}},
          React.createElement("div",{style:{color:"#8b949e",fontSize:10,fontWeight:700}},
            "🔑 مفتاح #"+(i+1),i===0?" (رئيسي)":i===1?" (احتياطي 1)":" (احتياطي 2)"),
        ...
        React.createElement("div",{style:{display:"flex",gap:6}},
          React.createElement("input",{type:shows[i]?"text":"password",value:vals[i],
            ...
            placeholder:i===0?"AIzaSy... (مطلوب)":"AIzaSy... (اختياري)",
            style:inpSt}),
    ...
    React.createElement("div",{style:{background:"#0d1117",border:"1px solid #30363d",borderRadius:10,padding:12}},
      React.createElement("div",{style:{color:"#e6edf3",fontWeight:700,fontSize:11,marginBottom:8}},"📖 كيف تحصل على مفتاح مجاني؟"),
      ["افتح aistudio.google.com","سجل دخول بحساب Google (يفضل 3 حسابات مختلفة)",'اضغط "Get API Key" ثم "Create API key"',"انسخ المفتاح والصقه هنا"].map(function(step,i){
