...
(function(){
  ...
  async function checkOnlineLicense(code) {
    try {
      ...
      if (!rows || !rows.length) return { ok: false, reason: t('الترخيص غير موجود في السيرفر') };
      const row = rows[0];
      if (row.frozen) return { ok: false, frozen: true, reason: t('تم تجميد هذا الترخيص من المدير') };
      if (row.expires && new Date(row.expires) < new Date()) return { ok: false, reason: t('انتهت صلاحية هذا الترخيص') };
      return { ok: true };
    } catch(e) {
...
function showFrozenScreen(reason){if(typeof window.__STOC_HIDE_LOADER__==="function")window.__STOC_HIDE_LOADER__();var ex=document.getElementById("stoc-frozen-screen");if(ex)ex.remove();var el=document.createElement("div");el.id="stoc-frozen-screen";el.style.cssText="position:fixed;inset:0;background:#0d1117;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:99999;font-family:Cairo,sans-serif;padding:32px;text-align:center;";el.innerHTML='<div style="font-size:56px;margin-bottom:16px">🔒</div>'+'<div style="color:#f85149;font-size:20px;font-weight:800;margin-bottom:10px">الحساب موقوف مؤقتاً</div>'+'<div style="color:#8b949e;font-size:13px;line-height:1.7;max-width:280px">'+(reason||"تم تجميد هذا الحساب من المدير. سيتم التحقق تلقائياً.")+"</div>"+'<div id="stoc-frozen-cd" style="color:#3fb950;font-size:12px;margin-top:18px"></div>';document.body.appendChild(el);var cnt=15,tries=0;var tm=setInterval(function(){cnt--;var cd=document.getElementById("stoc-frozen-cd");if(cd)cd.textContent=t('إعادة المحاولة خلال') + ' '+cnt+' ' + t('ثانية…');if(cnt<=0){cnt=Math.min(15+tries*5,60);tries++;if(cd)cd.textContent=t('جارٍ التحقق…');if(typeof window.__stkCheckLicense==="function"){window.__stkCheckLicense().then(function(ok){if(ok){clearInterval(tm);var sc=document.getElementById("stoc-frozen-screen");if(sc)sc.remove();}}).catch(function(){});}}},1000);}
  function showActivationScreen(initialMessage) {
    ...
    screen.innerHTML = `
      ...
      <div id="act-panel-qr">
        ...
        <div class="act-msg" id="act-msg">${t('اضغط لبدء المسح')}</div>
      ...
      <div id="act-panel-url" style="display:none;flex-direction:column;align-items:center;width:100%;max-width:380px;">
        ...
        <textarea id="act-url-input" placeholder=t('الصق الرابط هنا…')></textarea>
    ...
    async function processPayload(payload, setMsg) {
      if (!payload || !payload.sig || !payload.company || !payload.url || !payload.key) {
        throw new Error(t('الرابط غير صالح أو ناقص'));
      }
      const ok = await verify(payload);
      if (!ok) throw new Error(t('توقيع غير صحيح — الرابط غير معتمد'));
      if (isExpired(payload)) throw new Error(t('انتهت صلاحية هذا الترخيص'));

      if (navigator.onLine) {
        setMsg(t('جارٍ التحقق من السيرفر…'), "");
        const onlineCheck = await checkOnlineLicense(payload.code);
        if (!onlineCheck.ok) throw new Error(onlineCheck.reason);
      }

      setMsg(t('✅ تم التحقق — جارٍ تفعيل') + ' ' + payload.company, "ok");
      saveLicense(payload);
    ...
    window.__actStart = async function() {
      ...
      msg.textContent = t('جارٍ فتح الكاميرا…');
      try {
        ...
        if (window.__JSQR_FAILED__) {
          msg.textContent = t("⚠️ مسح QR غير متاح — استخدم خيار 'إدخال الرابط' بدلاً منه");
          return;
        }
        msg.textContent = t('وجّه الكاميرا نحو QR التفعيل…');
        scan();
      } catch(e) {
        msg.className = "act-msg err";
        msg.textContent = t('❌ تعذّر فتح الكاميرا:') + ' ' + e.message;
    ...
    window.__actStop = function() {
      ...
      if (msg) { msg.className = "act-msg"; msg.textContent = t('اضغط لبدء المسح'); }
    };
    ...
    async function processQR(raw) {
      ...
      try {
        ...
        msg.className = "act-msg err";
        msg.textContent = "❌ " + (e.message || t('QR غير صالح'));
        setTimeout(() => {
          msg.className = "act-msg";
          msg.textContent = t('اضغط لإعادة المحاولة');
          document.getElementById("act-start").style.display = "inline-block";
        }, 2500);
      }
    }
    ...
    window.__actSubmitURL = async function() {
      ...
      submitBtn.textContent = t('⏳ جارٍ التحقق…');
      urlMsg.className = "act-msg";
      urlMsg.textContent = t('جارٍ التحقق…');
      try {
        const payload = extractPayload(raw);
        await processPayload(payload, (text, cls) => {
        ...
        urlMsg.textContent = "❌ " + (e.message || t('رابط غير صالح'));
        submitBtn.disabled = false;
        submitBtn.textContent = t('✅ تفعيل');
      }
    };
  ...
  async function tryAutoActivateFromURL() {
    ...
    if (stocMsg) stocMsg.textContent = t('جارٍ تفعيل التطبيق…');
    ...
    try {
      ...
      if (!payload.sig || !payload.company || !payload.url || !payload.key) {
        throw new Error(t('رابط التفعيل غير صالح'));
      }

      const ok = await verify(payload);
      if (!ok) throw new Error(t('توقيع غير صحيح'));
      if (isExpired(payload)) throw new Error(t('انتهت صلاحية الترخيص'));

      if (navigator.onLine) {
        if (stocMsg) stocMsg.textContent = t('جارٍ التحقق من السيرفر…');
      ...
      if (stocMsg) stocMsg.textContent = t('✅ تم التفعيل —') + ' ' + payload.company;
      ...
      if (stocMsg) stocMsg.textContent = t('جارٍ التحميل…');
      return false;
    }
  }
