...
(function() {
  ...
  var DEMO_USER = {
    ...
    role_label:t('مدير عام'), role_icon:"👑",
    full_name:t('المدير'),
  ...
  window.fetch = function(input, init) {
    ...
    if (url === "/api/me")       return respond({ ok:true, user:DEMO_USER, permissions:{"*":true}, role_label:t('مدير عام') });
    ...
    if (url === "/api/factory-reset" && method === "POST") {
      if (pbody.password !== "0000") {
        return respond({ ok: false, error: t('كلمة السر غير صحيحة') }, 403);
      }
